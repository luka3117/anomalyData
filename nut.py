# %% [markdown]
# # ヘーゼルナッツ画像で異常検知を体験しよう
#
# この教材では、ヘーゼルナッツ画像を使って、
# **PyTorch の畳み込みオートエンコーダによる異常検知**を実践します。
#
# 学習目標
#
# 1. 正常画像・異常画像・正解マスクを読み込む
# 2. 正常画像だけでオートエンコーダを学習する
# 3. 再構成誤差から異常スコアを計算し、異常を判定する
# 4. 再構成誤差マップと正解マスクを比べ、異常箇所を確認する
#
# 想定するフォルダ構成
#
# ```text
...
# # 1. 実行環境を準備する
#
# `hazelnut` フォルダに移動してから実行してください。
#
# ```bash
# cd ~/D/d/G/l/l/d/hazelnut
# python ヘーゼルナッツ画像で異常検知を体験しよう.py
# ```
...
# hazelnut フォルダ内で実行するので、現在のフォルダをデータのルートとする。
# 外から実行する場合は Path("データの絶対パス") に変更する。
DATA_ROOT = Path.cwd()
...
# - `train/good`：学習用の正常画像
# - `test/good`：評価用の正常画像
# - `test/crack, cut, hole, print`：評価用の異常画像
# - `ground_truth/...`：異常箇所を示す正解マスク
...
# %% [markdown]
# # 4. Dataset と DataLoader を作る
#
# 画像を `128 × 128` のグレースケールに変換し、画素値を `0〜1` にします。
# ラベルは正常を `0`、異常を `1` とします。
...
# # 5. 畳み込みオートエンコーダを作る
#
# - **Encoder**：画像を小さな特徴表現に圧縮する
# - **Decoder**：特徴表現から画像を復元する
#
# 正常画像だけで学習すると、正常な模様は復元しやすくなります。
# 学習していない傷や穴は復元しにくくなることが期待されます。
...
# # 6. 正常画像だけで学習する
#
# 入力した正常画像と同じ画像を出力するように学習します。
# 講義中にCPUで短時間実行する場合は `EPOCHS = 20` 程度でも構いません。
...
# # 7. 再構成結果と誤差マップを見る
#
# 画素ごとの `|入力画像 − 再構成画像|²` を再構成誤差マップとします。
# 赤や黄色の部分ほど、モデルがうまく復元できなかったことを表します。
#
# オートエンコーダ自身の出力から直接得られるため、
# Grad-CAM や Integrated Gradients は使用しません。
...
# # 9. 異常検知の結果を評価する
#
# Accuracyだけでなく、ROC-AUC、Average Precision、異常の再現率も確認します。
...
# # 10. 正解マスクと誤差マップを比べる
#
# 例：`test/crack/000.png` の正解マスクは
# `ground_truth/crack/000_mask.png` です。
...
# %% [markdown]
# ## 画素単位のROC-AUCを計算する（発展）
#
# 誤差マップが正解の異常画素をどの程度上位に順位づけできるかを評価します。
...
# # 11. まとめ
#
# - `train/good` の正常画像だけで学習した
# - 入力画像と再構成画像の差を異常スコアにした
# - 学習用正常画像の95パーセンタイルをしきい値にした
# - 異常の種類ごとの検知率を比較した
# - 誤差マップと `ground_truth` の正解マスクを比較した
#
# 再構成誤差マップは異常判定の根拠を直感的に確認する可視化ですが、
# 正解マスクと完全に一致するとは限りません。
#
# 発展課題
#
# 1. `EPOCHS` を変える
# 2. `TOP_RATIO` を `0.01` や `0.10` に変える
# 3. しきい値を90・95・99パーセンタイルで比較する
# 4. RGB画像のまま学習した場合と比較する
